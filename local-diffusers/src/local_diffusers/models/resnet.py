...
class ResnetBlock3D(nn.Module):
    ...
    Parameters:
        ...
        time_embedding_norm (`str`, *optional*, default to `"default"` ): Time scale shift config.
            ...
            stronger conditioning with scale and shift.
        kernel (`torch.Tensor`, optional, default to None): FIR filter, see
            [`~models.resnet.FirUpsample2D`] and [`~models.resnet.FirDownsample2D`]. # TODO: This is a 2D kernel, should be changed to 3D
        output_scale_factor (`float`, *optional*, default to be `1.0`): the scale factor to use for the output.
        use_in_shortcut (`bool`, *optional*, default to `True`):
    ...
    def __init__(
        ...
        *,
        ...
        if time_embedding_norm == "ada_group":
            raise ValueError(
                "This class cannot be used with `time_embedding_norm==ada_group`, please use `ResnetBlockCondNorm3D` instead",
            )
        if time_embedding_norm == "spatial":
            raise ValueError(
                "This class cannot be used with `time_embedding_norm==spatial`, please use `ResnetBlockCondNorm3D` instead",
            )
        ...
        if self.up:
            if kernel == "fir":
                raise NotImplementedError("fir kernel is not implemented for 3D yet")
                fir_kernel = (1, 3, 3, 1)
...
# unet_rl.py
def rearrange_dims(tensor: torch.Tensor) -> torch.Tensor: #TODO: WATCH OUT, THIS WAS NOT CHANGED AT ALL, STILL 2D. if any calls on 3d there may be problems
