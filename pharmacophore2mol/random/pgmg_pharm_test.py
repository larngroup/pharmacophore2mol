...
def smiles2ppgraph(smiles:str, mol_conf):
    '''
    ...
    # mol_conf = mol # comment me

    fdefName = os.path.join(RDConfig.RDDataDir, 'BaseFeatures.fdef')
    factory = ChemicalFeatures.BuildFeatureFactory(fdefName)
    feats = factory.GetFeaturesForMol(mol_conf)
    for f in feats:
        phar = f.GetFamily()
        atom_index = f.GetAtomIds()
        atom_index = tuple(sorted(atom_index))
        atom_type = f.GetType()
        print(phar, atom_index, atom_type)
        print(f.GetPos())
...
if __name__ == '__main__':
    ...
    # smiles = 'C1=CC=CC=C1'
    smiles = 'c1ccccc1'
    suppl = Chem.SDMolSupplier("../data/raw/original_benzene.sdf", removeHs=True, sanitize=True, strictParsing=False)
    mol = suppl[0]
    smiles = Chem.MolToSmiles(mol)
    g_, smiles_code_res = smiles2ppgraph(smiles, mol)  ##g_药效团；smiles_code_res：smiles编码（矩阵格式）