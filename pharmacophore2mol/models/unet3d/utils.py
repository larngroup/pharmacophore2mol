...
def save_preds_as_gif(trues: np.ndarray, preds: np.ndarray, channel: int, filename: str, n_preds: int = 5):
    ...
    """
    fig_width = max(n_preds * 2, 10)  # Ensure the width is at least 10 to match the title width
    fig, axs = plt.subplots(2, n_preds, figsize=(fig_width, 4), dpi=100)
    if n_preds == 1:  # Ensure axs is always a 2D array
...
if __name__ == "__main__":
    ...
    save_preds_as_gif(preds, preds, channel=0, filename="./saves/predictions.gif", n_preds=1)